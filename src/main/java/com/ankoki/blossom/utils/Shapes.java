...
public class Shapes {
    ...
    /**
     * A method to get a list of locations to make up a sphere.
     *
     * @param centreBlock Define the center of the sphere
            ...
            for (int y = by - radius; y <= by + radius; y++) {
                for (int z = bz - radius; z <= bz + radius; z++) {
                    ...
                    if (distance < radius * radius && !(hollow && distance < ((radius - 1) * (radius - 1)))) {
                        Location loc = new Location(centreBlock.getWorld(), x, y, z);
